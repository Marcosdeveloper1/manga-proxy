  ...
  app.get('/search', async (req, res) => {
    ...
    // Mapeia palavras-chave para tipos de busca no MangaDex
    if (q.toLowerCase() === 'popular') { searchQuery = ''; orderBy = 'followedCount'; }
    if (q.toLowerCase() === 'trending') { searchQuery = ''; orderBy = 'rating'; }

    console.log(`[SEARCH] "${searchQuery}" source="${source || 'auto'}" lang="${lang || 'any'}" order="${orderBy}"`);
    ...
    if (source === 'mangadex') return res.json({ results: await mdxSearch(searchQuery, lang || null, orderBy), source: 'mangadex' });
    ...
    try {
      // Só busca no MangaPlus se não for busca por popular/trending (que MP não suporta bem via search)
      if (q.toLowerCase() !== 'popular' && q.toLowerCase() !== 'trending') {
    ...
    try {
      ...
      if (ck.length > 0) return res.json({ results: ck, source: 'comick' });
    } catch (e) {}

    try {
      const mdx = await mdxSearch(searchQuery, lang || 'pt-br', orderBy);
      if (mdx.length > 0) return res.json({ results: mdx, source: 'mangadex' });
    } catch (e) {}
